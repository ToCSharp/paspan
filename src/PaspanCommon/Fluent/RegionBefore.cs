...
public sealed class RegionBefore<D> : Parser<Region>
{
    ...
    public RegionBefore(Parser<D> delimiter, bool canBeEmpty = false, bool failOnEof = true, bool consumeDelimiter = false)
    {
        ...
        while (true)
        {
            ...
            if (reader.Eof())
            {
                ...
                if (length > 0)
                {
                    ...
                    result.Set(new Region(reader.GetPosition(start), previous2));
                }
                return true;
            }
...
public sealed class RegionBefore<D, S> : Parser<Region>
{
    private readonly Parser<D> _delimiter;
