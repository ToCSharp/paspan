...
public ref partial struct SpanReader
{
    ...
    {
        PartialStateForRollback rollBackState = CaptureState();
        while (_prevSpansLength + _consumed + count >= _sequence.Length)
        {
            if (IsLastSpan)
            {
                RollBackState(rollBackState);
    ...
    public void RollBackState(in PartialStateForRollback state/*, bool isError = false*/)
    {
        ...
        else
        {
            _prevSpansLength = 0;
            _consumed = 0;
            MoveConsumedTo((int)state._prevSpansLength + state._prevConsumed - (int)state._toAdvance);
        }
    }
    public PartialStateForRollback CaptureState()
    {
        return new PartialStateForRollback(_prevSpansLength, _consumed, _currentPosition, _updateBufferIteration, _toAdvance);
    }
    public readonly struct PartialStateForRollback
    {
        ...
        public readonly int _updateBufferIteration;
        public readonly long _toAdvance;

        public PartialStateForRollback(long prevSpansLength, int consumed, SequencePosition currentPosition, int updateBufferIteration, long toAdvance)
        {
            _prevSpansLength = prevSpansLength;
            _prevConsumed = consumed;
            _prevCurrentPosition = currentPosition;
            _updateBufferIteration = updateBufferIteration;
            _toAdvance = toAdvance;
        }
